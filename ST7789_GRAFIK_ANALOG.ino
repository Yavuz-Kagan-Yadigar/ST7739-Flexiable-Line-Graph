...
void grafik(int zaman, int max, int min, float aralik) {
  ...
  while (zaman * 1000 > (millis() - basla)) {  //grafik ve okuma
    tft.drawPixel(map((millis() - basla), 0, zaman * 1000, 0, ekran_x), map(analogRead(A0), min, max, 0, ekran_y), ST77XX_GREEN);//analogRead(A0)'ı grafiği çizilecek değişken/fonksiyonla değiştirin       replace analogRead(A0) with variable/function that you want to plot
